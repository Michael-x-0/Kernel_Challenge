...
def build_similarity_graph(X, var=1.0, eps=0.0, k=0):
    ...
    similarities = np.linalg.norm(X,axis=1).reshape(-1,1)**2+ -2*X@X.T + np.linalg.norm(X,axis=1).reshape(1,-1)**2
    similarities = np.exp(-similarities/(2*var))
    ...
    if k == 0:
        ...
        """
        W = similarities*(similarities >= eps).astype(int)
    ...
    # If kNN graph
        ...
        """
        W = similarities.copy()
        neighbors_idx = np.argsort(W, axis= 1)[:,:-k].flatten()
        line_idx = np.ones((n,n -k),dtype=np.int64)*np.arange(0,n).reshape(-1,1)
        W[line_idx.flatten(),neighbors_idx] = 0
